(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "bcrypt", "boom", "../models/User", "../utilities/createUser", "../utilities/userFunctions", "../utilities/authenticateUserSchema", "../utilities/token", "../utilities/sqliteUtilities"], factory);
    }
    ...
    const createUser_1 = require("../utilities/createUser");
    const userFunctions_1 = require("../utilities/userFunctions");
    const authenticateUserSchema_1 = require("../utilities/authenticateUserSchema");
    ...
    function hashPassword(password, cb) {
        ...
        bcrypt_1.genSalt(10, (err, salt) => {
            bcrypt_1.hash(password, salt, (err, hash) => {
                return cb(err, hash);
            });
        });
    }
            ...
            config: {
                ...
                handler: (req, res) => {
                    ...
                    hashPassword(newPassword, (err, hashedPassword) => {
                        ...
                        sqliteUtilities_1.getFullUsers(key).then(users => {
                            ...
                            newUser.key = key;
                            sqliteUtilities_1.updateUser(newUser).then(updatedUser => {
                                res(updatedUser);
                                return;
                            }).catch(error => {
                                console.log(error);
            ...
            config: {
                ...
                handler: (req, res) => {
                    ...
                    hashPassword(req.payload.password, (err, hash) => {
                        ...
                        user.password = hash;
                        sqliteUtilities_1.saveUser(user).then(success => {
                            if (success) {
                                res({
            ...
            config: {
                ...
                handler: (req, res) => {
                    ...
                    if (key === credentials.key) {
                        sqliteUtilities_1.getFullUsers(credentials.key).then(users => {
                            if (!users) {
                        ...
                        sqliteUtilities_1.getBasicUsers(key).then(users => {
                            if (key) {
                                let user = users[0];
                                if (!user) {
                                    throw Boom.badRequest("user key provided was not found");
